...
#  read shape
shp <- st_read("inputs/edge_hex_l5.shp", quiet = TRUE) %>% 
  st_sf()
...
quickmap <- tm_shape(edge) +
            tm_borders(col = 'blue') +
            tm_layout(title = 'Apps demo landscapes, Edge landscape (blue)')
...
for (file in unzipped_files) {
  ...
  new_file_path <- file.path("inputs", paste0("landfire_data_edge", file_extension))
...
stacked_rasters <- rast("inputs/landfire_data_edge.tif")
...
## Historical Vegetation Types-top 10 for the area of interest
...
## Existing Vegetation Types-top 10 for the area of interest
