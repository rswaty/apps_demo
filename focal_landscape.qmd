...
vdep_hexs <- focal_hex_filtered |>
  left_join(vdep_for_join, by = c("BPS_MODEL" = 'model_code'))


vdep_pal <- colorNumeric(
  palette = colorRampPalette(c("#1d522b", "white"))(100),
  domain = vdep_hexs$vdep
)
...
leaflet(vdep_hexs) |>
  ...
  fitBounds(lng1 = bbox_list$xmin, lat1 = bbox_list$ymin, lng2 = bbox_list$xmax, lat2 = bbox_list$ymax)  %>%
  addPolygons(
    fillColor = ~vdep_pal(vdep),
    color = "#BDBDC3",
    weight = 1,
    opacity = 1,
    fillOpacity = 1.0,
    popup = ~paste("VDEP:", vdep)) |>
  addLegend(
    pal = vdep_pal,
    values = vdep_hexs$vdep,
    title = "VDEP",
